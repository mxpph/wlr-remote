...
/*
 * My modifications:
 * - Add mdns_socket_callback_fn
 * - change mdns_answer_add_txt_record logic slightly
 */
...
typedef int (*mdns_socket_callback_fn)(int sock, void *buffer, size_t capacity,
...
//! Open and setup a IPv4 socket for mDNS/DNS-SD. To bind the socket to a
//! specific interface, pass in the appropriate socket address in saddr,
//! otherwise pass a null pointer for INADDR_ANY. To send one-shot discovery
//! requests and queries pass a null pointer or set 0 as port to assign a random
//! user level ephemeral port. To run discovery service listening for incoming
...
//! Setup an already opened IPv4 socket for mDNS/DNS-SD. To bind the socket to a
//! specific interface, pass in the appropriate socket address in saddr,
//! otherwise pass a null pointer for INADDR_ANY. To send one-shot discovery
//! requests and queries pass a null pointer or set 0 as port to assign a random
//! user level ephemeral port. To run discovery service listening for incoming
//! discoveries and queries, you must set MDNS_PORT as port.
static inline int mdns_socket_setup_ipv4(int sock,
                                         const struct sockaddr_in *saddr);

//! Open and setup a IPv6 socket for mDNS/DNS-SD. To bind the socket to a
//! specific interface, pass in the appropriate socket address in saddr,
//! otherwise pass a null pointer for in6addr_any. To send one-shot discovery
//! requests and queries pass a null pointer or set 0 as port to assign a random
//! user level ephemeral port. To run discovery service listening for incoming
//! discoveries and queries, you must set MDNS_PORT as port.
static inline int mdns_socket_open_ipv6(const struct sockaddr_in6 *saddr);

//! Setup an already opened IPv6 socket for mDNS/DNS-SD. To bind the socket to a
//! specific interface, pass in the appropriate socket address in saddr,
//! otherwise pass a null pointer for in6addr_any. To send one-shot discovery
//! requests and queries pass a null pointer or set 0 as port to assign a random
//! user level ephemeral port. To run discovery service listening for incoming
...
//! Listen for incoming multicast DNS-SD and mDNS query requests. The socket
//! should have been opened on port MDNS_PORT using one of the mdns open or
//! setup socket functions. Buffer must be 32 bit aligned. Parsing is stopped
//! when callback function returns non-zero. Returns the number of queries
//! parsed.
...
//! query ID set in a specific query. Any data will be piped to the given
//! callback for parsing. Buffer must be 32 bit aligned. Parsing is stopped when
//! callback function returns non-zero. Returns the number of responses parsed.
static inline size_t mdns_query_recv(int sock, void *buffer, size_t capacity,
                                     mdns_record_callback_fn callback,
                                     void *user_data, int query_id);

//! Send a variable unicast mDNS query answer to any question with variable
//! number of records to the given address. Use the top bit of the query class
//! field (MDNS_UNICAST_RESPONSE) in the query recieved to determine if the
//! answer should be sent unicast (bit set) or multicast (bit not set). Buffer
...
//! Send a variable multicast mDNS query answer to any question with variable
//! number of records. Use the top bit of the query class field
//! (MDNS_UNICAST_RESPONSE) in the query recieved to determine if the answer
...
//! Send a variable multicast mDNS announcement (as an unsolicited answer) with
...
//! Parse a SRV record, returns the priority, weight, port and name in the
//! record
...
mdns_record_parse_a(const void *buffer, size_t size, size_t offset,
                    size_t length, struct sockaddr_in *addr);
...
mdns_record_parse_aaaa(const void *buffer, size_t size, size_t offset,
                       size_t length, struct sockaddr_in6 *addr);

//! Parse a TXT record, returns the number of key=value records parsed and
//! stores the key-value pairs in the supplied buffer
static inline size_t mdns_record_parse_txt(const void *buffer, size_t size,
...
static inline mdns_string_t mdns_string_extract(const void *buffer, size_t size,
...
//! Compare if two strings are equal. If the strings are equal it returns >0 and
//! the offset variables are updated to the end of the corresponding strings. If
//! the strings are not equal it returns 0 and the offset variables are NOT
//! updated.
static inline int mdns_string_equal(const void *buffer_lhs, size_t size_lhs,
                                    size_t *ofs_lhs, const void *buffer_rhs,
...
static inline size_t mdns_string_table_find(mdns_string_table_t *string_table,
...
static inline int mdns_socket_setup_ipv4(int sock,
  ...
  if (!saddr) {
    ...
    setsockopt(sock, IPPROTO_IP, IP_MULTICAST_IF,
               (const char *)&sock_addr.sin_addr, sizeof(sock_addr.sin_addr));
#ifndef _WIN32
    sock_addr.sin_addr.s_addr = INADDR_ANY;
#endif
  }
...
static inline int mdns_socket_setup_ipv6(int sock,
  ...
  setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, (const char *)&reuseaddr,
             sizeof(reuseaddr));
#endif
  setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (const char *)&hops,
             sizeof(hops));
  setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (const char *)&loopback,
             sizeof(loopback));
  ...
  if (setsockopt(sock, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *)&req,
                 sizeof(req)))
    return -1;
  ...
  if (!saddr) {
    ...
    setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_IF, (const char *)&ifindex,
               sizeof(ifindex));
#ifndef _WIN32
    sock_addr.sin6_addr = in6addr_any;
#endif
...
static inline int mdns_string_equal(const void *buffer_lhs, size_t size_lhs,
  ...
  do {
    ...
    if (lhs_substr.length != rhs_substr.length)
      return 0;
    if (strncasecmp((const char *)MDNS_POINTER_OFFSET_CONST(buffer_rhs,
                                                            rhs_substr.offset),
                    (const char *)MDNS_POINTER_OFFSET_CONST(buffer_lhs,
                                                            lhs_substr.offset),
                    rhs_substr.length))
      return 0;
...
static inline void mdns_string_table_add(mdns_string_table_t *string_table,
                                         size_t offset) {
  if (!string_table)
    return;
...
static inline void *mdns_string_make_ref(void *data, size_t capacity,
                                         size_t ref_offset) {
  if (capacity < 2)
    return 0;
...
mdns_records_parse(int sock, const struct sockaddr *from, size_t addrlen,
                   const void *buffer, size_t size, size_t *offset,
                   mdns_entry_type_t type, uint16_t query_id, size_t records,
...
static inline size_t mdns_socket_listen(int sock, void *buffer, size_t capacity,
  ...
  for (int iquestion = 0; iquestion < questions; ++iquestion) {
    ...
    ++total_records;
    if (callback &&
        callback(sock, saddr, addrlen, MDNS_ENTRYTYPE_QUESTION, query_id, rtype,
...
static inline void *mdns_answer_add_question_unicast(
    void *buffer, size_t capacity, void *data, mdns_record_type_t record_type,
    const char *name, size_t name_length, mdns_string_table_t *string_table) {
  data =
      mdns_string_make(buffer, capacity, data, name, name_length, string_table);
  if (!data)
    return 0;
  size_t remain = capacity - MDNS_POINTER_DIFF(data, buffer);
  if (remain < 4)
...
static inline void *
mdns_answer_add_record_header(void *buffer, size_t capacity, void *data,
                              mdns_record_t record,
                              mdns_string_table_t *string_table) {
  data = mdns_string_make(buffer, capacity, data, record.name.str,
                          record.name.length, string_table);
  if (!data)
    return 0;
  size_t remain = capacity - MDNS_POINTER_DIFF(data, buffer);
  if (remain < 10)
...
static inline void *mdns_answer_add_record(void *buffer, size_t capacity,
                                           void *data, mdns_record_t record,
                                           mdns_string_table_t *string_table) {
  // TXT records will be coalesced into one record later
  if (!data || (record.type == MDNS_RECORDTYPE_TXT))
    return data;

  data = mdns_answer_add_record_header(buffer, capacity, data, record,
                                       string_table);
  if (!data)
    return 0;
...
static inline void mdns_record_update_rclass_ttl(mdns_record_t *record,
                                                 uint16_t rclass,
                                                 uint32_t ttl) {
  if (!record->rclass)
...
static inline void *
mdns_answer_add_txt_record(void *buffer, size_t capacity, void *data,
                           const mdns_record_t *records, size_t record_count,
                           uint16_t rclass, uint32_t ttl,
                           mdns_string_table_t *string_table) {
  // Pointer to length of record to be filled at end
  ...
  for (size_t irec = 0; data && (irec < record_count); ++irec) {
    ...
    // TXT strings are unlikely to be shared, just make then raw. Also need one
    // byte for termination, thus the <= check
    size_t string_length =
        record.data.txt.key.length + record.data.txt.value.length + 1;
    if (!data)
      return 0;
    remain = capacity - MDNS_POINTER_DIFF(data, buffer);
...
mdns_answer_get_record_count(const mdns_record_t *records,
                             size_t record_count) {
...
static inline int mdns_query_answer_unicast(
  ...
  // The cache-flush bit MUST NOT be set in any resource records in a response
  // message sent in legacy unicast responses to UDP ports other than 5353.
  ...
  header->authority_rrs =
      htons(mdns_answer_get_record_count(authority, authority_count));
  header->additional_rrs =
      htons(mdns_answer_get_record_count(additional, additional_count));

  mdns_string_table_t string_table = {{0}, 0, 0};
  void *data = MDNS_POINTER_OFFSET(buffer, sizeof(struct mdns_header_t));

  // Fill in question
  data = mdns_answer_add_question_unicast(buffer, capacity, data, record_type,
                                          name, name_length, &string_table);
  ...
  for (size_t irec = 0; data && (irec < authority_count); ++irec) {
    ...
    if (!record.ttl)
      record.ttl = ttl;
    data =
        mdns_answer_add_record(buffer, capacity, data, record, &string_table);
  }
  data =
      mdns_answer_add_txt_record(buffer, capacity, data, authority,
                                 authority_count, rclass, ttl, &string_table);
  ...
  data =
      mdns_answer_add_txt_record(buffer, capacity, data, additional,
                                 additional_count, rclass, ttl, &string_table);
  if (!data)
    return -1;
...
mdns_record_parse_srv(const void *buffer, size_t size, size_t offset,
  ...
  if ((size >= offset + length) && (length >= 8)) {
    const uint16_t *recorddata =
        (const uint16_t *)MDNS_POINTER_OFFSET_CONST(buffer, offset);
